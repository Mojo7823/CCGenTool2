...
export function buildSecurityProblemDefinitionHtml(
  ...
  const osp = data.osp ?? []
  const rootSectionNumber = options.rootSectionNumber ?? 2

  const assumptionsHeading = `${rootSectionNumber}.1`
  const threatsHeading = `${rootSectionNumber}.2`
  const ospHeading = `${rootSectionNumber}.3`
  ...
  html += `<h3>${assumptionsHeading} Assumptions</h3>`
  html +=
  ...
  html += `<h3>${threatsHeading} Threats</h3>`
  ...
  html += `<h3>${ospHeading} Organisational Security Policies</h3>`
  if (osp.length) {
    html +=
