...
async function generatePreview() {
  ...
  try {
    ...
    let coverImagePath = coverData?.uploadedImagePath ?? null
    if (coverData && userToken.value) {
      const previousPath = coverImagePath
      const { path, uploaded } = await ensureCoverImagePath({
        userToken: userToken.value,
        imagePath: coverImagePath,
        imageData: coverData.uploadedImageData,
        fileName: 'final-preview-cover',
      })

      coverImagePath = path
      coverData.uploadedImagePath = coverImagePath

      if (uploaded || coverImagePath !== previousPath) {
        sessionService.saveCoverData(coverData.form, coverImagePath, coverData.uploadedImageData || null)
      }
    }
