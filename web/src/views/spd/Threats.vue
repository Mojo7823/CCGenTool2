    ...
    <div class="card spd-menubar">
      <div class="spd-menubar-left">
        ...
        <span class="menubar-subtitle">
          Write the threats addressed by the TOE and its environment.
        </span>
      </div>
      <div class="spd-actions">
        ...
        <button class="btn info" type="button" @click="openPreviewModal" :disabled="!items.length">
          Preview
        </button>
      </div>
    </div>
        ...
        <table class="spd-table">
          ...
          <tbody>
            ...
            <tr v-if="!items.length">
              <td class="no-data" colspan="2">No threats have been added yet.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <label class="modal-label" for="threat-name">Threat</label>
          <input
            id="threat-name"
            v-model="form.title"
            class="modal-input"
            type="text"
            placeholder="Insert Threat"
          />

          <label class="modal-label" for="threat-description">Description</label>
          <RichTextEditor
...
const modalTitle = computed(() => (isEditing.value ? 'Edit Threat' : 'Insert Threat'))
const primaryButtonLabel = computed(() =>
  isEditing.value ? 'Save Changes' : 'Finalize and Add Threat'
)
const selectedIdExists = computed(() => selectedId.value !== null)

function loadSessionData() {
  const data: ThreatsSessionData | null = sessionService.loadThreatsData()
  if (data) {
...
function persistSession() {
  sessionService.saveThreatsData([...items.value], nextId.value)
}
...
function finalizeEntry() {
  if (!form.title.trim()) {
    validationError.value = 'Threat name is required.'
    return
  }
...
function removeSelected() {
  ...
  const entry = items.value.find(item => item.id === selectedId.value)
  if (!entry) return
  const confirmed = window.confirm(`Remove threat "${entry.title}"?`)
  if (!confirmed) return
