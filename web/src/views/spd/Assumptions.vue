        ...
        <table class="spd-table">
          ...
          <tbody>
            ...
            <tr v-if="!items.length">
              <td class="no-data" colspan="2">No assumptions have been added yet.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <label class="modal-label" for="assumption-name">Assumptions</label>
          <input
            id="assumption-name"
            v-model="form.title"
            class="modal-input"
          ...
          <label class="modal-label" for="assumption-description">Description</label>
          <RichTextEditor
...
const modalTitle = computed(() => (isEditing.value ? 'Edit Assumptions' : 'Insert Assumptions'))
const primaryButtonLabel = computed(() =>
  isEditing.value ? 'Save Changes' : 'Finalize and Add Assumptions'
)
const selectedIdExists = computed(() => selectedId.value !== null)

function loadSessionData() {
  const data: AssumptionsSessionData | null = sessionService.loadAssumptionsData()
  if (data) {
...
function persistSession() {
  sessionService.saveAssumptionsData([...items.value], nextId.value)
}
...
function finalizeEntry() {
  if (!form.title.trim()) {
    validationError.value = 'Assumptions name is required.'
    return
  }
...
function removeSelected() {
  ...
  const entry = items.value.find(item => item.id === selectedId.value)
  if (!entry) return
  const confirmed = window.confirm(`Remove assumption "${entry.title}"?`)
  if (!confirmed) return
