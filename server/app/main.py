...
def _decode_base64_image(image_base64: Optional[str]) -> Optional[BytesIO]:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid base64-encoded cover image provided")
...
def _get_cover_image_source(
    image_path: Optional[str], image_base64: Optional[str], user_id: str
) -> Optional[Union[Path, BytesIO]]:
    base64_stream = _decode_base64_image(image_base64)
    if base64_stream is not None:
...
def _add_cover_image(document: Document, image_source: Optional[Union[Path, BytesIO]]):
...
def _build_cover_document(payload: CoverPreviewRequest) -> Path:
    image_source = _get_cover_image_source(payload.image_path, payload.image_base64, payload.user_id)
    ...
    _add_cover_image(document, image_source)
...
def _build_st_intro_combined_document(payload: STIntroPreviewRequest) -> Path:
    ...
    if payload.cover_data:
        ...
        try:
            image_source = _get_cover_image_source(
                cover_dict.get("image_path"), cover_dict.get("image_base64"), payload.user_id
            )
...
def _build_final_combined_document(payload: FinalPreviewRequest) -> Path:
    ...
    has_security_sections = bool(payload.sfr_list) or bool(payload.sar_list)

    if has_conformance or has_security_sections:
        document.add_page_break()
    ...
    if has_conformance:
        ...
        conf_run = conf_heading.add_run(f"{section_number}. Conformance Claims")
        conf_run.font.size = Pt(20)
        conf_run.font.bold = True
        conf_heading.space_before = Pt(12)
        conf_heading.space_after = Pt(8)
        _append_html_to_document(document, payload.conformance_claims_html or "")
        section_number += 1
    ...
    if has_security_sections:
        ...
        sec_run = sec_heading.add_run(f"{section_number}. Security Requirements")
        sec_run.font.size = Pt(20)
        sec_run.font.bold = True
        sec_heading.space_before = Pt(12)
        sec_heading.space_after = Pt(12)

        sub_index = 1

        if payload.sfr_list:
            sfr_heading = document.add_paragraph()
            sfr_run = sfr_heading.add_run(f"{section_number}.{sub_index} Security Functional Requirements")
            sfr_run.font.size = Pt(18)
            sfr_run.font.bold = True
            sfr_heading.space_before = Pt(8)
            sfr_heading.space_after = Pt(8)
        ...
        if payload.sar_list:
            ...
            sar_run = sar_heading.add_run(f"{section_number}.{sub_index} Security Assurance Requirements")
            sar_run.font.size = Pt(18)
            sar_run.font.bold = True
            sar_heading.space_before = Pt(8)
            sar_heading.space_after = Pt(8)
